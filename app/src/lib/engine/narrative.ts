// narrative.ts — composes the clinical narrative summary (HTML string) from
// the collected aura-symptom-check answers. Ported 1:1 from the Hugo
// summaryGeneration.js; behaviour-preserving. `lang` is passed explicitly
// (defaults to <html lang> in the browser) so the composer is testable headless.

import { formatList, getPronouns } from "./helpers";
import { t } from "./i18n";
import type {
  AcuteChronicAnswers,
  AcuteChronicData,
...
export interface NarrativeInput {
  ...
  /** Explicit locale; falls back to <html lang> in the browser. */
  lang?: Lang;
}

/**
 * Generates a narrative summary based on the provided data.
 * @returns the generated narrative summary as an HTML string.
 */
export function generateNarrativeSummary({
  ...
  const modalityMap: Record<string, string> = {
    ...
    speech: t("flow.summary.modality.speech", "speech/language disturbances"),
  ...
  Object.entries(modalityMap).forEach(([key, label]) => {
    ...
    const values = answers
      ...
      .map((v) => (key === "retinal" && v === "yes" ? visionLossNote : v));
  ...
  const auraCharacteristicsDetails: string[] = [];
  const auraCharacteristicsNotApply: string[] = [];
